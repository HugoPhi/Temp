...
def visualize_clusters(data, true_labels, predicted_labels, centroids):
    ...
    plt.suptitle(f"Clustering By K-means after PCA to {num_features}", fontsize=16)
    plt.tight_layout()
    plt.savefig(f'./res/iris_pca_{num_features}.png')
    # plt.show()
...
for pca2 in [4, 3, 2]:
    ...
    reduced_train, reduced_test, labels_train, labels_test = train_test_split(
        reduced_data, true_labels, test_size=0.3, random_state=42)
    ...
    # Visualize clusters on training set
    visualize_clusters(reduced_train, labels_train, predicted_train_labels, centroids)

    # use SVC to check accuracy
    from sklearn.svm import SVC
    svc = SVC()
    svc.fit(reduced_train, labels_train)
    svc_pred = svc.predict(reduced_test)

    svc_train_accuracy = accuracy_score(labels_train, svc.predict(reduced_train))
    svc_test_accuracy = accuracy_score(labels_test, svc_pred)
